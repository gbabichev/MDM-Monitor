...
final class AppUpdateCenter: ObservableObject {
    ...
    private let checker = RepositoryTagUpdateChecker()
...
private struct AppUpdateConfiguration: Sendable {
    enum HostingProvider: Sendable {
        case github
        case gitea(baseURL: URL)
    }
    ...
    nonisolated static func current(bundle: Bundle = .main) -> AppUpdateConfiguration? {
        ...
        guard
            ...
            let parsedConfiguration = repositoryConfiguration(from: releasesPageURL)
        else {
            return nil
        }
        ...
        return AppUpdateConfiguration(
            ...
            provider: parsedConfiguration.provider,
            owner: parsedConfiguration.owner,
            repository: parsedConfiguration.repository,
            releasesPageURL: releasesPageURL
        )
    }

    private nonisolated static func repositoryConfiguration(
        from url: URL
    ) -> (provider: HostingProvider, owner: String, repository: String)? {
        guard
        ...
        guard let baseURL = URL(string: "\(scheme)://\(host)\(url.port.map { ":\($0)" } ?? "")") else {
            return nil
        }

        return (.gitea(baseURL: baseURL), owner, repository)
    }
}

/// Lightweight repository tags API client used for update checks.
private struct RepositoryTagUpdateChecker {
    private struct Tag: Decodable, Sendable {
        let name: String
    }
    ...
    func latestTagName(
        ...
        repository: String,
        userAgent: String,
        provider: AppUpdateConfiguration.HostingProvider
    ) async throws -> String {
        let url: URL
